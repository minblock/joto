...
###   This script attempts to download the signature file SHA256SUMS.asc from joto.org
...
RCSUBDIR="test/"
BASEDIR="https://joto.org/bin/"
VERSIONPREFIX="joto-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'joto-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the JOTO Core binary release signing key installed?"
   fi
