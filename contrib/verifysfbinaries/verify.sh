...
###   This script attempts to download the signature file SHA256SUMS.asc from jotocoin.org
...
WORKINGDIR="/tmp/jotocoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://jotocoin.org/bin/"
VERSIONPREFIX="jotocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'jotocoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the JOTOCOIN Core binary release signing key installed?"
   fi
