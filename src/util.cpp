...
const char * const JOTOCOIN_CONF_FILENAME = "jotocoin.conf";
const char * const JOTOCOIN_PID_FILENAME = "jotocoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "jotocoin" is a composite category enabling all JOTOCOIN-related debug output
            if(ptrCategory->count(string("jotocoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JOTOCOINCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JOTOCOINCore
    // Mac: ~/Library/Application Support/JOTOCOINCore
    // Unix: ~/.jotocoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JOTOCOINCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JOTOCOINCore";
#else
    // Unix
    return pathRet / ".jotocoincore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", JOTOCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty jotocoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override jotocoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", JOTOCOIN_PID_FILENAME));
