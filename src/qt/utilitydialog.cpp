    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("JOTOCOIN Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About JOTOCOIN Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  jotocoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_JOTOCOIN_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", JOTOCOINGUI::DEFAULT_UIPLATFORM));
        }
...
All the JOTOCOIN in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 JOTOCOIN, 0.1 JOTOCOIN, 1 JOTOCOIN and 10 JOTOCOIN -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("JOTOCOIN Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(JOTOCOINGUI *window)
{
    if (!window)
        return nullptr;
