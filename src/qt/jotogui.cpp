...
const std::string JOTOGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString JOTOGUI::DEFAULT_WALLET = "~Default";

JOTOGUI::JOTOGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/joto_logo_horizontal");
...
void JOTOGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a JOTO address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and joto: URIs)"));
    ...
    aboutAction = new QAction(tr("&About JOTO"), this);
    aboutAction->setStatusTip(tr("Show information about JOTO"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for JOTO"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your JOTO addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified JOTO addresses"));
    ...
    openAction->setStatusTip(tr("Open a joto: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the JOTO help message to get a list with possible JOTO command-line options"));
...
void JOTOGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void JOTOGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool JOTOGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool JOTOGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void JOTOGUI::setWalletActionsEnabled(bool enabled)
{
...
void JOTOGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("JOTO client") + " " + networkStyle->getTitleAddText();
...
void JOTOGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void JOTOGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void JOTOGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void JOTOGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void JOTOGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void JOTOGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void JOTOGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void JOTOGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void JOTOGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void JOTOGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void JOTOGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void JOTOGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to JOTO network", "", count));
    } else {
...
void JOTOGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void JOTOGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void JOTOGUI::updateHeadersSyncProgressLabel()
{
...
void JOTOGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void JOTOGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void JOTOGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("JOTO"); // default title
...
void JOTOGUI::changeEvent(QEvent *e)
{
...
void JOTOGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void JOTOGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void JOTOGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(JOTOUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void JOTOGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void JOTOGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool JOTOGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool JOTOGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void JOTOGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void JOTOGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void JOTOGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void JOTOGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void JOTOGUI::detectShutdown()
{
...
void JOTOGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void JOTOGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void JOTOGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(JOTOGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void JOTOGUI::subscribeToCoreSignals()
{
...
void JOTOGUI::unsubscribeFromCoreSignals()
{
...
void JOTOGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void JOTOGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<JOTOUnits::Unit> units = JOTOUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const JOTOUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(JOTOUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(JOTOUnits::Unit u, JOTOUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(JOTOUnits::name(u)), this);
...
{
    setText(JOTOUnits::name(newUnits));
}
